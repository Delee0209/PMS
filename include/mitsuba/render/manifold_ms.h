...
public:
    ...
    struct PathPhoton {
        Point3f p;                              // this should be the same as the last vertex location
        EmitterInteraction ei;                  // where path originate
        int depth;
        Point3f path[32];
        // data for ReIR SMS
        Point3f s;
    ...
    std::vector<PathPhoton *> causticPhotons;
    PhotonReservoir r;

    // helper for photon initialization
    bool m_photonInit = false;
    int m_photonDepth;
    int m_photonIndex;
    bool m_randomPhoton;
    EmitterInteraction m_ei;
    ...
    // function for shooting photon
    void caustic_photon_Shooting(ref<Sampler> sampler);
