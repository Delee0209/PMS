...
struct SMSConfig {
    ...
    // Parameters for Photon-driven Manifold Sampling
    bool photon_reflection = false;				// force all specular interaction to be reflection
    int photon_count = 10000;					// photon count
    int random_mode = 0;                        // 0 -> clostest, 1 -> assign random photon, 2 -> clostest by chance
    bool photon_jitter = true;					// enabling photon purterbation
	float photon_cone = 15.f;					// purterbation cone angle
    float update_prob  = 0.5f;					// the probability u for selecting photon (only use when random mode == 2)
