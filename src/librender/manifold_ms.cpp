...
// shoot photon, basically doing the specular manifold seed path initialization in reverse (from light source)
MTS_VARIANT void SpecularManifoldMultiScatter<Float, Spectrum>::caustic_photon_Shooting(ref<Sampler> sampler) {
    std::cout << "Photon Pass" << std::endl;
    std::vector<PathPhoton *> tmp;
    causticPhotons = tmp;
    // Gather all the potential caustic caster
    auto shapes = m_scene->caustic_casters_multi_scatter();
    if (unlikely(shapes.size() == 0)) {
        return;
    }

    SurfaceInteraction3f dummy;

    // photon tracing
    while(causticPhotons.size() < m_config.photon_count) {
        // initialize path photon structure
        PathPhoton * proposalPhoton = new PathPhoton();
        
        //proposalPhoton.path.clear();
        proposalPhoton->depth = 0;

        // Sample emitter interaction as the starting point for photon
        proposalPhoton->ei = SpecularManifold::sample_emitter_interaction(dummy, m_scene->caustic_emitters_multi_scatter(), sampler);
        Point3f x0 = proposalPhoton->ei.p;

        // sample photon direction, sample the first vertex on a randomly selected specular shape
        int shape_idx =  min(UInt32(sampler->next_1d() * (ScalarFloat) shapes.size()), (uint32_t) shapes.size() - 1);
        const ShapePtr first_shape = shapes[shape_idx];
        auto ps = first_shape->sample_position(dummy.time, sampler->next_2d());
        Point3f x1 = ps.p;
        Vector3f wo = normalize(x1 - x0);
        Ray3f ray(x0, wo, dummy.time, dummy.wavelengths);

        bool Valid = false;

        while (true) {
            int bounce = proposalPhoton->depth;
            if (bounce >= 32) {
                /* reaching maximum depth for photon, yet to reach any causic receiver, abandon path */
                break;
            }

            SurfaceInteraction3f si = m_scene->ray_intersect(ray);
            ...
            if (m_config.photon_reflection) {
                vertex.eta = 1.f;
            }
            proposalPhoton->path[proposalPhoton->depth] = si.p;
            proposalPhoton->depth ++;
            ...
            // On first intersect, make sure that we hit a specualr vertex (caustic photon)
            if (bounce == 0 && (!shape->is_caustic_caster_multi_scatter() && !shape->is_caustic_bouncer())) {
                break;
            }

            if (shape->is_caustic_receiver()) {
                // We hit the first caustic recevier we terminate the path and store the photon
                proposalPhoton->p = si.p;
                causticPhotons.push_back(proposalPhoton);
                Valid = true;
                break;
            }
        ...
        if (!Valid) {
            delete(proposalPhoton);
        }
    }
    std::cout << "Full with " << causticPhotons.size()<< std::endl;
}
...
SpecularManifoldMultiScatter<Float, Spectrum>::specular_manifold_sampling(const SurfaceInteraction3f &si,
    ...
    m_ei = SpecularManifold::sample_emitter_interaction(si, m_scene->caustic_emitters_multi_scatter(), sampler);
    //EmitterInteraction ei = SpecularManifold::sample_emitter_interaction(si, m_scene->caustic_emitters_multi_scatter(), sampler);
    //EmitterInteraction ei = causticPhotons[0].ei;
    ...
    Spectrum result(0.f);
    for (size_t shape_idx = 0; shape_idx < 1; ++shape_idx) { // here just a dunny for loop, as PMS does not need to loop through casters
        const ShapePtr specular_shape = shapes[shape_idx]; // well keep this because I don't want to change class layout :)
        ...
        if (!m_config.biased) {
            ...
            bool success = sample_path(specular_shape, si, m_ei, sampler, true);
            if (!success) {
                stats_solver_failed++;
                continue;
            }
            ...
            Spectrum specular_val = evaluate_path_contribution(si, m_ei);
            ...
            while (true) {
                ...
                bool success_trial = sample_path(specular_shape, si, m_ei, sampler, false);
            ...
            for (int m = 0; m < m_config.max_trials; ++m) {
                ...
                bool success = sample_path(specular_shape, si, m_ei, sampler,
                                           m_seed_path.size() == 0);
                if (!success) {
                    stats_solver_failed++;
                    continue;
...
SpecularManifoldMultiScatter<Float, Spectrum>::sample_path(const ShapePtr shape,
    ...
    Mask success_seed = sample_seed_path(shape, si, m_ei, sampler, first_path, p_start);
    if (!success_seed) {
        return false;
    }
    ...
    Mask success_solve = newton_solver(si, m_ei);
    if (!success_solve) {
        return false;
    }
...
MTS_VARIANT typename SpecularManifoldMultiScatter<Float, Spectrum>::Mask
SpecularManifoldMultiScatter<Float, Spectrum>::sample_seed_path(const ShapePtr shape_,
    ...
    if (m_config.mnee_init) {
        Complex<Spectrum> ior = shape_->bsdf()->ior(si);
        Mask reflection = any(neq(0.f, imag(ior)));
        if (reflection) {
            // Modified "MNEE"
            BoundingBox3f bbox = shape_->bbox();
    ...
    int photonDepth = 0, photonDepthCounter = 0, photonIndex = 0;
    bool photonInit = false;
    Float min_dis   = 50.0;
    // init reservoir
    r.photonIndex = 0;
    r.weightSum = 0;
    r.m         = 0;
    { // sampling photon
        if (m_config.random_mode == 0) { // use closest photon as inital seed path
            for (int i = 0; i < causticPhotons.size(); i++) {
                Vector3f dis = x0 - causticPhotons[i]->p;
                if (dot(dis, dis) < min_dis) { // overwrite seed path
                    min_dis     = dot(dis, dis);
                    photonInit  = true;
                    photonIndex = i;
                }
            }
        } else if (m_config.random_mode == 1) { // use random photon as initial seed path
            photonInit  = true;
            photonIndex = int(sampler->next_1d() * (causticPhotons.size() - 1));
            m_randomPhoton = true;
        } else if (m_config.random_mode == 2) { // use closest photon by chance
            photonInit  = true;
            photonIndex = int(sampler->next_1d() * (causticPhotons.size() - 1)); // initialize to random photon as fallback
            for (int i = 0; i < causticPhotons.size(); i++) {
                Vector3f dis = x0 - causticPhotons[i]->p;
                if (dot(dis, dis) < min_dis &&
                    sampler->next_1d() <= m_config.update_prob) {
                    min_dis     = dot(dis, dis);
                    photonIndex = i;
                }
            }
        } else if (m_config.random_mode == 3) { // RIS distance
            photonInit = true;
            for (int i = 0; i < causticPhotons.size(); i++) {
                Vector3f dis = x0 - causticPhotons[i]->p;
                if (min_dis < dot(dis, dis))
                    continue;
                Float weight = cos(1 - dot(dis, dis) / min_dis) + 1;
                weight *= weight;
                weight *= (8 - causticPhotons[i]->depth);
                // weight *= weight;
                // update reservoir
                r.weightSum += weight;
                r.m++;
                if (sampler->next_1d() < (weight / r.weightSum)) {
                    r.photonIndex = i;
                }
            }
            photonIndex = r.photonIndex;
        }
    }

    if (photonInit) {
        m_photonInit  = true;
        m_photonDepth      = causticPhotons[photonIndex]->depth;
        m_photonIndex      = photonIndex;
        photonDepth        = causticPhotons[photonIndex]->depth;
        photonDepthCounter = 2;
        if (m_config.photon_jitter) {
            Vector3f dis    = x0 - causticPhotons[photonIndex]->p;
            min_dis      = sqrt(dot(dis, dis));
            Vector3f jitter = Vector3f(sampler->next_1d()-0.5f, sampler->next_1d()-0.5f, sampler->next_1d()-0.5f);
            wo = normalize(causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter] - x0 + normalize(jitter) * min_dis);
        } else {
            wo = normalize(causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter] - x0);
        }
    }
    ...
    while (true) {
        ...
        if (bounce >= m_config.bounces || photonDepthCounter > photonDepth) {
        ...
        // Perform scattering at vertex, unless we are doing the straight-line MNEE initialization
        if (!m_config.mnee_init) {// || !photonInit) {
            // Get current (potentially offset) normal in world space
            Vector3f m = vertex.s * n_offset[0] +
                         vertex.t * n_offset[1] +
                         vertex.n * n_offset[2];
        ...
        if (photonInit) {
            photonDepthCounter ++;
            if (m_config.photon_jitter) { // photon path purterbation
                Vector3f jitter = Vector3f(sampler->next_1d()-0.5f, sampler->next_1d()-0.5f, sampler->next_1d()-0.5f);
                float jitter_cone = 1 / tan((m_config.photon_cone / 180.f) * 3.14159);
                wo = normalize(jitter_cone*normalize(causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter] - causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter + 1]) + normalize(jitter));
            } else {
                wo = normalize(causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter] - causticPhotons[photonIndex]->path[photonDepth - photonDepthCounter + 1]);
            }
        }
