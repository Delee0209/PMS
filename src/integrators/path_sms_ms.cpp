...
public:
    ...
    SpecularManifoldMultiScatter PhotonCaster;
protected:
...
public:
    MultiScatterSMSPathIntegrator(const Properties &props) : Base(props) {
        ...
        m_sms_config.uniqueness_threshold   = props.float_("uniqueness_threshold", 1e-5f);
        m_sms_config.max_trials             = props.int_("max_trials", -1);

        m_sms_config.bounces                = props.int_("bounces", 8);

        m_sms_config.photon_count           = props.int_("photon_count", 2500);
        m_sms_config.random_mode            = props.int_("random_mode", 2);
        m_sms_config.photon_jitter          = props.bool_("photon_jitter", false);
		m_sms_config.photon_cone			= props.float_("photon_cone", 15.f);
        m_sms_config.photon_reflection      = props.bool_("photon_reflection", false);
        m_sms_config.update_prob            = props.float_("update_prob", 0.75f);

        m_biased_mnee                  = props.bool_("biased_mnee", false);
    }

    bool render(Scene *scene, Sensor *sensor) override {
		// start photon pass before the main path tracing pass
        ref<Sampler> sampler = sensor->sampler();
        PhotonCaster.init(scene, m_sms_config);
        PhotonCaster.caustic_photon_Shooting(sampler);
        ...
        if constexpr (is_array_v<Float>) {
            ...
            for (int depth = 1;; ++depth) {
                ...
                if (si.shape->is_caustic_receiver() && !on_caustic_caster &&
                    ...
                    mf.causticPhotons = PhotonCaster.causticPhotons;
                ...
                if (emitter) {
                    ...
                    if (!on_caustic_caster ||
                        specular_camera_path && emitter->is_environment()) { // emitter->is_environment() filter out the glint cause by hitting light
